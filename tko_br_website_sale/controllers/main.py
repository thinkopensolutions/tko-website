...
class CorreiosL10nBrWebsiteSale(main.WebsiteSale):
    ...
    def cart(self, **post):
        ...
        valid = True
        if ICPSudo.get_param('tko_br_website_sale.sale_freight_warning') == 's' and len(set([line.product_id.has_frete for line in order.order_line if line.product_id])) > 1:
            valid = False
